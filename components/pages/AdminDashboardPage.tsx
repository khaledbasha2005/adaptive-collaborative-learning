...
const getCognitiveLevel = (score: number | null, totalQuestions: number): string => {
  ...
  // Using percentage for more robust thresholds
  const percentage = (score / totalQuestions) * 100;

  if (percentage <= 33) return 'مبتدئ';
  if (percentage <= 66) return 'متوسط';
  return 'متقدم';
};


const AdminDashboardPage: React.FC<AdminDashboardPageProps> = ({ groups, allStudents, allModuleScores, quizQuestionsByModule }) => {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-800 mb-6">لوحة تحكم الباحثة - تحليلات الطلاب والمجموعات</h1>
      
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        {/* Chart for Group Cognitive Level */}
        <div className="bg-white p-6 rounded-xl shadow-lg">
          <h2 className="text-xl font-bold text-gray-700 mb-4">عدد المجموعات حسب المستوى المعرفي</h2>
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg">
            <h2 className="text-xl font-bold text-gray-700 mb-4">المسارات التعليمية للطلاب</h2>
